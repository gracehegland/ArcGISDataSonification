...
# Read in the data that has been exported from an ArcGIS attribute table as a
# CSV file
...
# Create a dictionary with longitude and latitude pairs as the keys
# and notes as the values
latLongNotes = dict(
    zip(zip(data[XLongColumnName], data[YLatColumnName]), midi_data)
)

# Sort the dictionary first by decreasing latitude and then by increasing
# longitude
sortedLatLongNotes = dict(
    sorted(latLongNotes.items(), key=lambda item: (-item[0][1], item[0][0]))
)
...
sortedByLong = dict(
    sorted(latLongNotes.items(), key=lambda item: (item[0][0]))
)
...
def in_range(latlong_and_note, bandWidth):
    """
    Returns whether the latitude is in the specified range of the current
    latitude (global) and the longitude is in the range set by the zoom
    This range will define the width of our latitude bands when used to filter
    sortedLatLongNotes
    """
    ...
    return (
        lat <= currentLat
        and lat > currentLat - bandWidth
        and long >= longrange[0]
        and long <= longrange[1]
    )


def say_line(line_counter):
    "Reads the current line number"
...
def say_help_info():
    ...
    HELP_TEXT = (
        "Tab plays a line. The left arrow decreases the speed of lines. The"
        " right arrow increases the speed of lines. The down arrow moves to"
        " the next line. The up arrow moves to the previous line. One allows"
        " you to zoom in on the first half of a region. Two allows you to zoom"
        " in on the second half of a region. Zero returns the zoom to the"
        " default. Z tells you the zoom level of the current region. S takes"
        " you to the starting line. M takes you to the middle line. E takes"
        " you to the ending line. I tells you what line you are currently on."
    )
...
def say_zoom_info(zoom_counter):
    ...
    ZTEXT = (
        f"The zoom level is {calculate_zoom_percentage(zoom_counter)} percent"
    )
...
def play_sine_wave(lineDuration):
    ...
    sinewave = SineWave(
        pitch=0, pitch_per_second=5 / lineDuration
    )  # Starting pitch = 0
    sinewave.play()
...
def notesToDelays(sortedLatLongNotes, delayTime, intervalWidth):
    ...
    for i in range(len(sortedLatLongNotes) - 1):
        ...
        delays.append(
            delayCalc(longs[i], longs[i + 1], delayTime, intervalWidth)
        )
    return delays


def midi_play(note, noteDuration):
    # 0x94 is the status byte for the "note on" message,
    # note pitch, note velocity (volume) = 127
    note_on = [0x94, note, 127]
    # 0x84 is the status byte for the "note off" message,
    # note pitch, note velocity (volume) = 0
...
def play_midi_notes(sortedLatLongNotes, noteDuration, delayTime):
    ...
    for i in range(len(sortedLatLongNotes)):
        ...
        if i < len(timeDelays):
            # If there are corresponding time delays, wait before playing the
            # next note
            time.sleep(timeDelays[i])


def edgeDelays(longInitial, longFinal, delayTime, intervalWidth):
    # Calculate the length of the delay between the start of the line and
    # the first note
    initialDelay = ((longInitial - longitudes[0]) / intervalWidth) * delayTime
    # Calculate the length of the delay between the last note and the end of
    # the line
...
def linePlay(currentLat, beep_frequency, lineDuration, noteTime):
    ...
    latitudeBands = dict(
        filter(
            lambda item: in_range(item, bandWidth), sortedLatLongNotes.items()
        )
    )
    # Sort the latitude bands by increasing longitude
    sortedLatitudeBands = dict(
        sorted(latitudeBands.items(), key=lambda item: item[0])
    )
    ...
    if noteCount > 0:
        # Use threading to simultaneously play the sine wave and
        # the piano notes
        sine_thread = Thread(target=play_sine_wave, args=(lineDuration,))
        sine_thread.start()

        noteDuration = noteTime / noteCount
        initialDelay, finalDelay = edgeDelays(
            longs[0], longs[noteCount - 1], delayTime, intervalWidth
        )
...
def on_key_release(key):
    # TODO: Why must all these be global?
    # Can we encapsulate inside a better data structure?
    global currentLat, beep_frequency, line_counter, longrange, zoom_counter
    global bandWidth, lineDuration, noteTime

    if (
        currentLat < lats[-1]
    ):  # if current latitude is less than the minimum latitude
    ...
    try:
        ...
        if key.char == "1":
            longrange[1] = longrange[0] + (
                (longrange[1] - longrange[0]) / 2
            )  # Set a new upperbound
            zoom_counter += 1
        ...
        # Non-character events don't have a "char" attribute.
...
bandWidth = (
    lats[0] - lats[-1]
) / 60  # Fix the line number at 60 and calculate the bandwidth
lineDuration = 10
noteTime = 5
